    ...
    <div class="row align-items-center">
        ...
        <div class="col-12 order-md-1">
            <div class="d-flex align-items-center gap-3 justify-content-between">
                <div>
                    <h3>Laporan Hutang Piutang</h3>
                    <p class="text-subtitle text-muted">Manage Laporan Hutang Piutang</p>
                </div>
                <div>
                    <a href="{{route('report.accountsReceivableAndPayable.print', ['start_date' => request()->get('start_date'), 'end_date' => request()->get('end_date') ])}}" class="btn btn-sm mb-1 d-flex align-items-center gap-2 btn-danger block" 
                        >
                      ...
                      <tbody>
                          @foreach ($receivables as $entry)
                              <tr>
                                  ...
                                  <td style="font-size: 13px; "><div class="me-2 d-inline-block mb-1 p-1 px-2 bg-light-primary rounded-3" style="font-size: 10px;">{{$entry->account->no_code}}</div>{{ $entry->account->parent()->account_name .' / '. $entry->account->account_name }}</td>
                                  <td style="font-size: 13px">{{ $entry->name ?? $entry->description }}</td>
                                  <td style="font-size: 13px">{{ formatRupiah($entry->debit) }}</td>
                                  <td style="font-size: 13px">{{ formatRupiah($entry->credit) }}</td>
                              </tr>
                          @endforeach
                          <tr>
                            <td style="font-size: 13px; background: #f7f7f7" colspan="3">Total</td>
                            <td style="font-size: 13px; background: #ffdede">{{formatRupiah($receivables->sum('debit'))}}</td>
                            <td style="font-size: 13px; background: #ddffe5">{{formatRupiah($receivables->sum('credit'))}}</td>
                          </tr>
                      </tbody>
                  </table>
                </div>
                ...
                <div class="card-body">
                  ...
                  <table class="table overflow-hidden rounded-4">
                      ...
                      <tbody>
                          ...
                          <tr>
                            <td style="font-size: 13px; background: #f7f7f7" colspan="3">Total</td>
                            <td style="font-size: 13px; background: #ddffe5">{{formatRupiah($payables->sum('debit'))}}</td>
                            <td style="font-size: 13px; background: #ffdede">{{formatRupiah($payables->sum('credit'))}}</td>
                          </tr>
                      </tbody>
                  </table>
                </div>
