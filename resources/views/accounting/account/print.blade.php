...
<div class="d-flex align-items-center justify-content-center position-fixed bottom-0 end-0 start-0 mb-2">
    <div class="d-flex align-items-center gap-1 bg-white shadow-sm p-1 rounded-pill">
        <a href="{{route('invoice.index')}}" class="btn p-2 px-3 btn-light-secondary rounded-pill">Back</a>
        ...
        <div class="row">
            <div class="col-7">
                <div class="p-1">
                    <table class="mb-2 w-100">
                        <tr>
                            <td width="60">Nomor</td><td width="10" class="pl-1 pr-1">:</td><td>{{$invoice->no}}</td>
                        </tr>
                        <tr>
                            <td width="60">Lampiran</td><td width="10" class="pl-1 pr-1">:</td><td>-</td>
                        </tr>
                        <tr>
                            <td width="60">Perihal</td><td width="10" class="pl-1 pr-1">:</td><td><span class="previewPerihal">{{$invoice->perihal}}</span></td>
                        </tr>
                        <tr>
                            <td width="60" class="align-top">Kepada</td>
                            <td width="10" class="pl-1 pr-1 align-top">:</td>
                            <td>
                                <span class="previewClientName fw-bold">{{$invoice->client->name}}</span><br>
                                <span class="previewClientAddress">{{$invoice->client->addresses->keyBy('address_tag')->get('office')->address}}</span><br>
                                <span class="previewClientPostal">{{$invoice->client->addresses->keyBy('address_tag')->get('office')->postal_code}}</span>
                            </td>
                        </tr>
                        <tr>
                            <td width="60">U.P.</td><td width="10" class="pl-1 pr-1">:</td><td><span class="previewFor">{{$invoice->for}}</span></td>
                        </tr>
                    </table>
                </div>
            </div>
            <div class="col-5">
                <div class="p-1 text-end">
                    Jakarta, <span class="previewDate">{{formatTanggal($invoice->date)}}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="print-body mb-4 ml-5 mr-5">
        ...
        <div class="table-responsive mb-4">
            <table class="table mb-0 table-lg" id="previewTable">
                ...
                <tbody>
                    {{-- Product Looping --}}
                    @foreach (json_decode($invoice->purchaseOrder->products) as $product)
                    <tr>
