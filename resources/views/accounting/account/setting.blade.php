@extends('templates.crud.add', ['routeName'=>'account'])

@section('title', 'Setting Account')
@section('description', 'Setting Account : '.$account->account_name)
...
@section('form')
            ...
            <div class="row">
                ...
                <div class="col-md-3 text-end">
                    <button type="button" class="btn btn-sm btn-outline-danger block d-flex align-items-center gap-2 mb-1 ms-auto" data-bs-toggle="modal"
                        data-bs-target="#danger-{{$account->id}}">
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable"
                        ...
                        <div class="modal-content rounded-4">
                            ...
                            <div class="modal-body border-0">
                                Apakah anda yakin menghapus data Account <strong> {{$account->account_name}}</strong> ?
                            </div>
    ...
    <div class="d-flex align-items-center gap-4">
        <h4 class="mb-0">Data Journal Entries yang terkait</h4>
        <div class="d-flex align-items-center justify-content-center bg-primary text-white rounded-5 p-2" style="aspect-ratio:1/1 !impportant; width:2em">{{count($account->entries)}}</div>
    </div>
    ...
    @forelse ($account->entries as $entry)
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="d-flex align-items-center justify-content-between">
                            <div class="w-100">
                                <h6 class="m-0">{{$entry->name ?? $entry->description}}</h6>
                                <p class="m-0">{{ \Carbon\Carbon::parse($entry->created_at)->translatedFormat('d F Y') }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="text-end">
                            ...
                            <div class="fs-6 m-0">Debit : {{formatRupiah($entry->debit)}}</div>
                            <div class="fs-6 m-0">Credit : {{formatRupiah($entry->credit)}}</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="d-flex align-items-center jutify-content-center p-5">
            <div>
                Belum ada Product yang menggunakan satuan ini, <a href="{{route('account.index')}}">Lihat Journal Entries</a>
            </div>
        </div>
    @endforelse
@endsection
