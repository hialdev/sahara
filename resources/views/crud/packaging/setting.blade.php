@extends('templates.crud.add', ['routeName'=>'packaging'])

@section('title', 'Setting Packaging')
@section('description', 'Setting Packaging : '.$packaging->name)
...
@section('form')
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-9">
                    ...
                    <h6 class="mb-0">{{$packaging->name}}</h6>
                    <p class="mb-0" style="font-size: 12px">Capacity : {{$packaging->capacity}} {{$packaging->satuan?->name}}</p>
                </div>
                <div class="col-md-3 text-end">
                    <button type="button" class="btn btn-sm btn-outline-danger block d-flex align-items-center gap-2 mb-1 ms-auto" data-bs-toggle="modal"
                        data-bs-target="#danger-{{$packaging->id}}">
                    ...
                    <p class="mb-0" style="font-size: 12px"><span class="text-danger">*</span> Mohon perhatikan effek yang ditimbulkan dari menghapus packaging, seperti transaksi Purchase Order yang terkait dengan packaging ini.</p>
                </div>
                {{-- Modal Delete --}}
                <div class="modal fade text-left" id="danger-{{$packaging->id}}" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable"
                        ...
                        <div class="modal-content rounded-4">
                            ...
                            <div class="modal-body border-0">
                                Apakah anda yakin menghapus data packaging <strong> {{$packaging->name}}</strong> ?
                            </div>
    ...
    <div class="d-flex align-items-center gap-4">
        ...
        <div class="d-flex align-items-center justify-content-center bg-primary text-white rounded-5 p-2" style="aspect-ratio:1/1 !impportant; width:2em">{{count($packaging->purchaseOrders)}}</div>
    </div>

    @forelse ($packaging->purchaseOrders as $processed)
        <div class="card mb-2">
            <div class="card-body">
                    ...
                    <div class="accordion-item">
                        <div class="accordion-header" id="flush-headingOne">
                            <button class="accordion-button collapsed rounded-3 border" type="button" data-bs-toggle="collapse" data-bs-target="#flush-{{$processed->id}}" aria-expanded="false" aria-controls="flush-{{$processed->id}}">
                                {{count($processed->getProducts)}} packaging yang diproses
                            </button>
                        </div>
                        <div id="flush-{{$processed->id}}" class="accordion-collapse collapse" aria-labelledby="flush-headingOne" data-bs-parent="#accordionProcessed-{{$processed->id}}">
                            <div class="p-3">
                                ...
                                <div class="d-flex align-items-center justify-content-between">
                                    <div>
                                        <h6>{{$prod->packaging->title}}</h6>
                                        <p style="font-size:13px">{{$prod->packaging->description}}</p>
                                    </div>
                                    <div>
                                        <h6>{{$prod->qty ?? 0}}</h6>
