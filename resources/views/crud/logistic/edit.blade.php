@extends('templates.crud.add', ['routeName'=>'logistic'])

@section('title', 'Edit logistic : '.$logistic->name)
@section('description', 'Edit logistic and Addresses')

@section('form')
<div>
    <ul class="nav nav-tabs mb-3" id="myTab" role="tablist">
        <li class="nav-item" role="presentation">
            <a class="nav-link active" id="logistic-tab" data-bs-toggle="tab" href="#logistic" role="tab"
                aria-controls="logistic" aria-selected="true">Logistic</a>
        </li>
        <li class="nav-item" role="presentation">
            <a class="nav-link" id="address-tab" data-bs-toggle="tab" href="#address" role="tab"
                aria-controls="address" aria-selected="false">Logistic Address</a>
        </li>
    </ul>
    <div class="tab-content" id="myTabContent">
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <input type="text" id="name" name="name" required class="form-control" value="{{old('name', $logistic->name)}}" placeholder="logistic or Supplier Name">
                                </div>
                                <div class="mb-3">
                                    <label for="email" class="form-label">Email <span class="text-danger">*</span></label>
                                    <input type="email" id="email" name="email" required class="form-control" value="{{old('email', $logistic->email)}}" placeholder="email@mail.com">
                                </div>
                                <div class="mb-3">
                                    <label for="description" class="form-label">Description</label>
                                    <textarea name="description" id="description" cols="30" rows="3" class="form-control">{{old('description', $logistic->description)}}</textarea>
                                </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <input type="text" id="name" name="contact_name" required value="{{old('contact_name', $logistic->contact_name)}}" class="form-control" placeholder="PIC Name">
                                </div>
                                <div class="mb-3">
                                    <label for="email" class="form-label">Email <span class="text-danger">*</span></label>
                                    <input type="email" id="email" name="contact_email" required value="{{old('contact_email', $logistic->contact_email)}}" class="form-control" placeholder="pic@email.com">
                                </div>
                                <div class="">
                                    <label for="tel" class="form-label">Phone <span class="text-danger">*</span></label>
                                    <input type="tel" id="tel" name="contact_phone" required value="{{old('contact_phone', $logistic->contact_phone)}}" class="form-control" placeholder="No Telp">
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="tab-pane fade" id="address" role="tabpanel" aria-labelledby="address-tab">
            <div class="row">
                                        ...
                                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable"
                                            ...
                                            <div class="modal-content rounded-4">
                                                ...
                                                <div class="modal-body border-0">
                                                    Apakah anda yakin menghapus data alamat pada logistic {{$logistic->name}} dengan id {{$logistic->id}} ? data yang dihapus bersifat permanen tidak dapat dikembalikan
                                                </div>
                                                <div class="modal-footer border-0">
                                                    ...
                                                    <form action="{{route('logistic.address.destroy', ['logistic_id' => $logistic->id, 'id' => $address->id])}}" method="POST">
                                                        @csrf
                                                        @method('delete')
                ...
                <div class="col-md-6">
                    <form action="{{route('logistic.address.manage', ['logistic_id' => $logistic->id])}}" method="POST">
                        @csrf
...
<script>
    $(document).ready(function () {
        ...
        $('.btn-edit-address').click(function () {
            ...
            if(addressTag == 'office'){
                $('#address_tag').prop('disabled', true)
            }
