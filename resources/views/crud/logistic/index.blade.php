            ...
            <div class="col-6 col-md-12">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-start">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Logistic</li>
                    </ol>
                </nav>
            </div>
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>Logistic</h3>
                <p class="text-subtitle text-muted">Manage Logistic</p>
            </div>
            <div class="col-6 order-md-2 order-1">
                <a href="{{route('logistic.add')}}" class="btn btn-primary float-end">Add New</a>
            </div>
        </div>
    </div>
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        <button type="button" class="btn btn-sm btn-outline-secondary d-flex align-items-center justify-content-center" style="aspect-ratio:1/1;" data-bs-toggle="modal"
                                            data-bs-target="#addressModal-{{$logistic->id}}">
                                        ...
                                        {{-- Modal Address --}}
                                        <div class="modal fade" id="addressModal-{{$logistic->id}}" tabindex="-1" role="dialog"
                                            ...
                                            <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable"
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body border-0 py-0">
                                                        @forelse ($logistic->addresses as $address)
                                                        ...
                                                        @empty
                                                        Address not found for this logistic
                                                        @endforelse
                                                    </div>
                                        ...
                                        <div>
                                            <div class="fw-semibold">{{$logistic->name}}</div>
                                            <div style="font-size:12px" class="text-secondary">{{$logistic->email}}</div>
                                        </div>
                                    </div>
                                </td>
                                <td style="max-width: 10em; font-size:12px">{{$logistic->description}}</td>
                                <td>
                                    <div class="fw-semibold">
                                        {{$logistic->contact_name}}
                                    </div>
                                    <div style="font-size:12px" class="text-secondary">{{$logistic->contact_email}}, {{$logistic->contact_phone}}</div>
                                </td>
