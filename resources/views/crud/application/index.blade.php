    ...
    <div class="page-title">
        <div class="row align-items-center">
            ...
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>Applications</h3>
                <p class="text-subtitle text-muted">Manage Applications and place an icon to make more beautifull</p>
            </div>
            <div class="col-6 order-md-2 order-1">
                <a href="{{route('application.add')}}" class="btn btn-primary float-end">Add New</a>
            </div>
        </div>
    </div>
                ...
                <div class="table-responsive datatable-minimal">
                    <table class="table" id="table2">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <div data-id="{{$item->id}}" class="id-application text-decoration-underline text-primary rounded-3 p-1 px-2" style="cursor: pointer; font-size:12px">{{$item->id}}</div>
                                </td>                                
                                @foreach ($columns as $column)
                                @php
                                    $value = $column['name'];
                                ...
                                <td style="width: 5em">
                                    <div class="d-flex align-items-center gap-1">
                                        <a href="{{route('application.edit', $item->id)}}" class="btn text-primary d-flex align-items-center justify-content-center p-1 rounded-2">
                                        ...
                                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable"
                                            ...
                                            <div class="modal-content rounded-4">
                                                ...
                                                <div class="modal-body border-0">
                                                    Apakah anda yakin menghapus data application dengan id {{$item->id}} ? data yang dihapus bersifat permanen tidak dapat dikembalikan
                                                </div>
...
<script>
    $(document).ready(function() {
        ...
        // Event listener untuk klik pada elemen dengan id "id-application"
        $(".id-application").on("click", function() {
            // Ambil nilai dari data-id di elemen yang diklik
            const textToCopy = $(this).data('id');

            // Panggil fungsi salin ke clipboard
        ...
        function copyToClipboard(text) {
            ...
            // Isi elemen input dengan teks yang ingin disalin
            aux.setAttribute("value", text);

            // Tambahkan elemen input ke body
            document.body.appendChild(aux);

            // Sorot kontennya
            aux.select();

            // Salin teks yang disorot
            const successful = document.execCommand("copy");

            // Hapus elemen input dari body
            document.body.removeChild(aux);

            // Tampilkan toast berdasarkan hasil penyalinan
            if (successful) {
                Toast.fire({
                    icon: 'success',
                    title: 'Berhasil copy ID Application'
                });
            } else {
                Toast.fire({
                    icon: 'error',
                    title: 'Gagal menyalin ID Application'
                });
            }
        }
    });
