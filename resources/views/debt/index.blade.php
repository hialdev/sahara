            ...
            <div class="col-6 col-md-12">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-start">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Debts (Hutang)</li>
                    </ol>
                </nav>
            </div>
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>Debts (Hutang)</h3>
                <p class="text-subtitle text-muted">Manage Debt / Hutang ke Principle</p>
            </div>
        </div>
    </div>
                            ...
                            <tr>
                                <td>
                                    <div style="font-size:13px">
                                        <span class="fw-bold" >{{$debt->no}}</span><br/>
                                        {{$debt->date}}
                                    </div>
                                </td>
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        <button type="button" class="btn btn-sm btn-outline-secondary d-flex align-items-center justify-content-center" style="aspect-ratio:1/1;" data-bs-toggle="modal"
                                            data-bs-target="#addressModal-{{$debt->id}}">
                                        ...
                                        {{-- Modal Products --}}
                                        <div class="modal fade" id="addressModal-{{$debt->id}}" tabindex="-1" role="dialog"
                                            aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable"
                                                role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header border-0 ">
                                                        <h5 class="modal-title" id="exampleModalCenterTitle">Process Order Details</h5>
                                                    ...
                                                    <div class="modal-body border-0 py-0">
                                                        ...
                                                        <div class="w-100 p-2 rounded-3 bg-primary text-white mb-3" style="display: -webkit-box;
                                                        ...
                                                        <div>
                                                            <div class="d-flex align-items-center gap-3 mb-3">
                                                                ...
                                                                <h6 class="m-0" style="font-size: 13px">Detail Proses PO</h6>
                                                            </div>
                                                            <div class="p-3 rounded-3 bg-light-primary">
                                                                @if (isset($debt->processOrder->logistic))
                                                                <div style="font-size: 12px" class="fw-semibold">Dikirim oleh</div>
                                                                <div>{{$debt->processOrder->logistic->name}}</div>
                                                                
                                                                <div class="mt-2">
                                                                    <div style="font-size: 12px" class="fw-semibold">Jemput Pesanan di</div>

                                                                    <div class="fw-semibold">{{$debt->processOrder->address->address_tag}}</div>
                                                                    <div style="font-size:12px" class="text-muted">Address : {{$debt->processOrder->address->address}}, {{$debt->processOrder->address->city}} - {{$debt->processOrder->address->postal_code}}</div>
                                                                    <div style="font-size:12px" class="text-muted">TELP / FAX : {{$debt->processOrder->address->telp}} / {{$debt->processOrder->address->fax}}</div>
                                                                </div>
                                                                @endif

                                                                <div class="mt-2">
                                                                    <div style="font-size: 12px" class="fw-semibold">Kirim ke</div>

                                                                    <div class="fw-semibold">{{$debt->processOrder->purchaseOrder->address->address_tag}}</div>
                                                                    <div style="font-size:12px" class="text-muted">Address : {{$debt->processOrder->purchaseOrder->address->address}}, {{$debt->processOrder->purchaseOrder->address->city}} - {{$debt->processOrder->purchaseOrder->address->postal_code}}</div>
                                                                    <div style="font-size:12px" class="text-muted">TELP / FAX : {{$debt->processOrder->purchaseOrder->address->telp}} / {{$debt->processOrder->purchaseOrder->address->fax}}</div>
                                                                </div>
                                                            </div>
                                        ...
                                        <div>
                                            <div class="fw-semibold">
                                                @if($debt->principle)
                                                    {{ $debt->principle->trashed() ? '[Deleted] ' : '' }}{{ $debt->principle->name }}
                                                @else
                                                    <span class="text-danger">Principle Not Found</span>
                                                @endif
                                            </div>
                                            <div style="font-size:12px" class="text-secondary">
                                                @if($debt->principle)
                                                    {{ $debt->principle->email }}
                                                @else
                                ...
                                <td>
                                    ...
                                    <button type="button" class="border-0 outline-0 bg-transparent text-primary gap-2 d-flex align-items-center justify-content-center" data-bs-toggle="modal"
                                        data-bs-target="#productModal-{{$debt->id}}">
                                        ...
                                        <span style="white-space:nowrap">{{count($debt->processOrder->getProducts)}} Products</span>
                                    </button>
                                    {{-- Modal Address --}}
                                    <div class="modal fade" id="productModal-{{$debt->id}}" tabindex="-1" role="dialog"
                                        aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable"
                                            role="document">
                                            <div class="modal-content">
                                                <div class="modal-header border-0 ">
                                                    <h5 class="modal-title" id="exampleModalCenterTitle">Processed Products </h5>
                                                    <button type="button" class="btn text-secondary" data-bs-dismiss="modal"
                                                        aria-label="Close">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 20 20">
                                                            <path fill="currentColor" d="M10 0c5.523 0 10 4.477 10 10s-4.477 10-10 10S0 15.523 0 10S4.477 0 10 0m2.207 6.837L10.01 9.03L7.815 6.837a.68.68 0 0 0-.88-.072l-.084.072a.68.68 0 0 0 0 .964l2.195 2.193l-2.195 2.193a.682.682 0 1 0 .964.965l2.195-2.195l2.197 2.195c.24.24.613.263.88.071l.084-.072a.68.68 0 0 0 0-.964l-2.196-2.193l2.195-2.193a.682.682 0 0 0-.963-.964" />
                                                ...
                                                <div class="modal-body border-0 py-0">
                                                    @forelse ($debt->processOrder->getProducts as $product)
                                                    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-3 p-3 border rounded-3">
                                                        ...
                                                        <div class="ms-md-auto text-end">
                                                            <div class="fw-bold">{{ formatRupiah($product->price_buy) }} / {{$product->product->satuan->name}}</div>
                                ...
                                <td style="width: 5em">
                                    <div class="d-flex align-items-center gap-1">
                                        <a href="{{route('purchase.show', $debt->processOrder->purchaseOrder->id)}}" title="Go to Process Order Detail" class="btn btn-sm btn-success block" style="aspect-ratio:1/1">
                                        ...
                                        <a href="{{route('debt.print', $debt->id)}}" title="Print" class="btn btn-sm btn-danger block" style="aspect-ratio:1/1">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                                        ...
                                        <a href="{{route('debt.show', $debt->id)}}" class="btn btn-sm btn-primary block" title="Setting Debt" style="aspect-ratio:1/1">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 20 20">
